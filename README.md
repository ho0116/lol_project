...
### 3.3 소결
이 모델은 특정 날짜에 대해서는 높은 정확도를 보이고 있지만, 전체적인 안정성이 부족하다는 한계를 가지고 있습니다. 정확도가 높은 플래티넘 티어 예측에 대해서는 신뢰할 만한 결과를 제공하고 있지만, 전체적인 정확도에서는 더 많은 개선이 필요합니다.  
프로젝트에서는 KDA, DPM, DPG, DPD, DTPM과 같은 주요 데이터를 활용하여 플레이어의 전반적인 활약, 고딜 기여, 자원 활용, 생존 능력, 맵 컨트롤을 파악하고, 이를 기반으로 승패를 예측한다. 각 데이터 간의 상관관계를 분석하고 중요한 데이터를 식별하여 모델을 조정하는 전략을 채택한다.
## 4. 결론 및 배운점
CNN을 히고 싶어서 시도해 봤는데 0915 데이터를 돌릴 때마다 loss가 nan으로 나와서, 시간의 제약으로 인해 더 이상 진행할 수 없었지만 다른 날짜들은 잘 돌아간다는 것을 알 수 있었다. 0825 데이터는 플래티넘 ↑ : 95.8%, 전체 : 94.2%, 단순 합산으로 예측했을 땐 좀 낮게 나왔던 0917 데이터는 플래티넘 ↑ : 92.8%, 전체 : 92.8%로 CNN이 높게 나왔다는 것을 알 수 있었다. 다음에는 시간 분배를 더욱 체계적으로 해서 원하는 결과를 얻을 수 있도록 할 것이다. 리드미를 작성할 때 롤에 대해 관심이 있어서 배경지식이 많았다면 조금 더 수월하지 않았을까 하는 생각이 든다.
