...
## 1. 개 요
League Of Lengend(이하 LOL)는 중국 텐센트 산하의 Riot Games가 개발한 MOBA 장르의 게임이다. MOBA 게임의 진입장벽을 낮춤으로 높은 인기를 얻어 전 세계에서 즐기는 대표적인 컴퓨터 게임이 되었다. LOL은 e-sports 산업에서 대표적인 콘텐츠로 자리매김하고 있으며, 2022년 항저우 아시안 게임에서는 정식 종목으로 채택될 만큼 그 영향력은 이미 검증되어 있는 사실이다. <a href="https://biz.chosun.com/site/data/html_dir/2020/12/17/2020121702229.html">[1]</a>
...
## 3. 승패예측 모델
### 3.1 모델 개요
각 팀의 핵심 데이터를 합산하고 정규화한 후, 승패를 예측하는 모델을 두 가지 방법으로 구현해봤다. 하나는 단순합산 방식이고, 다른 하나는 CNN을 활용한 방식이다.
### 3.2 성능
<table>
  ...
  <tr align="center"><th rowspan="2">0915</th><td>플래티넘 ↑</td><td>93.4%</td><td>94.3%</td></tr>
  <tr align="center"><td>전체</td><td>90.3%</td><td>93.4%</td></tr>
  <tr align="center"><th rowspan="2">0917</th><td>플래티넘 ↑</td><td>88.4%</td><td>92.8%</td></tr>
  <tr align="center"><td>전체</td><td>86.2%</td><td>92.8%</td></tr>
</table>

### 3.3 소결
티어가 플래티넘 이상이거나 전체에서도 CNN이 단순합산보다 더 높은 정확도를 보이고 있다. 이를 통해 핵심데이터를 합산하고 정규화한 후에 CNN을 사용하는 것이 승패 예측에 더 효과적임을 알 수 있다. 
팀의 전투 참여도, 공격력, 생존 능력, 방어 능력 등을 종합적으로 평가하고, 이를 바탕으로 승패를 예측했다. 그 결과, CNN 방식이 더 높은 정확도를 보이는 것으로 보아, 핵심 데이터들은 승패 예측에 유의미한 영향을 끼쳤음을 알 수 있다.
## 4. 결론 및 배운점
티어를 플래티넘 이상으로 할 때나 전체로 할 때 모두, CNN이 단순합산 방식보다 더 높은 정확도를 보여주었다. 이번 프로잭트에서는 전반적인 흐름이나 전략적인 측면 관련 속성들을 고려하지 않고 전투 관련 지표로만 핵심 데이터 특성을 설정하였음에도 불구하고, 높은 성능을 얻을 수 있었다는 점을 확인하였다. 이를 통해, 향후 승패 예측 모델 개발 시에는 CNN과 같은 딥러닝 기법의 활용이 중요하다는 결론을 도출할 수 있었다.
또한 롤에 대한 배경 지식이 풍부하다면 리드미 작성이 더욱 수월했을 것이라는 생각이 들었다. 특히, 구체적인 게임 메커니즘과 플레이 스타일에 대한 이해는 핵심 데이터 특성 선택에 큰 도움이 되었을 것이다.
